class NotifyMailer < ActionMailer::Base
  ...
  def notify_email(who, alert)
    @who = who
    @alert = alert
    @n = NotifyControl.first(:conditions => "serial = '#{alert.notify_control.serial}' and model = '#{alert.notify_control.model}' and name = '#{alert.notify_control.name}'")
    ...
    unless @last_sent.nil?
      @num_past_alerts = Alert.find( :all, :conditions => ['alert_msg = ? and name = ? and alert_date > ? and alert_date < ?', alert.alert_msg, alert.notify_control.name, @last_sent, alert.alert_date]).count
    end
    mail(:to => who, :subject => "#{alert.notify_control.name} - Alert Message - #{alert.alert_msg}")
    @n.save
  end
